...
function vitePluginCesium(options = {
    ...
    return {
            ...
            if (!isBuild) {
                ...
                if (rebuildCesium) {
                    ...
                    userConfig.build = {
                        rollupOptions: {
                            external: ['cesium'],
                            plugins: [rollup_plugin_external_globals_1.default({ cesium: 'Cesium' })]
                        }
                    };
                }
            }
        ...
        configureServer({ middlewares }) {
            ...
            middlewares.use(CESIUM_BASE_URL, serve_static_1.default(cesiumPath));
        },
        async closeBundle() {
            if (isBuild) {
                try {
        ...
        transformIndexHtml() {
            const tags = [
                {
                    ...
                    attrs: {
                        rel: 'stylesheet',
                        href: vite_1.normalizePath(path_1.default.join(CESIUM_BASE_URL, 'Widgets/widgets.css'))
                    }
                }
            ];
            if (isBuild && !rebuildCesium) {
                tags.push({
                    tag: 'script',
                    attrs: { src: vite_1.normalizePath(path_1.default.join(base, 'cesium/Cesium.js')) }
                });
            }
            return tags;
        }
